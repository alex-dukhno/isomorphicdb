...
fn update_number() {
    ...
    assert_eq!(
        analyzer.analyze(update_statement(
            vec![SCHEMA, TABLE],
            vec![("col", ast::Expr::Value(number(1)))]
        )),
...
fn update_with_column_value() {
    ...
    assert_eq!(
        analyzer.analyze(update_statement(
            vec![SCHEMA, TABLE],
            vec![("col_1", ast::Expr::Identifier(ident("col_2")))]
        )),
        Ok(QueryAnalysis::Write(Write::Update(UpdateQuery {
            full_table_id: FullTableId::from((schema_id, table_id)),
            sql_types: vec![SqlType::SmallInt],
...
fn update_with_column_value_that_does_not_exists() {
    ...
    assert_eq!(
        analyzer.analyze(update_statement(
            vec![SCHEMA, TABLE],
            vec![("col_1", ast::Expr::Identifier(ident("col_3")))]
        )),
...
mod implicit_cast {
    ...
    fn string_to_boolean() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                vec![SCHEMA, TABLE],
                vec![("col", ast::Expr::Value(ast::Value::SingleQuotedString("t".to_owned())))]
            )),
            Ok(QueryAnalysis::Write(Write::Update(UpdateQuery {
                full_table_id: FullTableId::from((schema_id, table_id)),
                sql_types: vec![SqlType::Bool],
    ...
    fn boolean_to_string() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                vec![SCHEMA, TABLE],
                vec![("col", ast::Expr::Value(ast::Value::Boolean(true)))]
            )),
            Ok(QueryAnalysis::Write(Write::Update(UpdateQuery {
                full_table_id: FullTableId::from((schema_id, table_id)),
    ...
    fn string_to_number() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                ...
                vec![(
                    "col",
                    ast::Expr::Value(ast::Value::SingleQuotedString("100".to_owned()))
                )]
            )),
            Ok(QueryAnalysis::Write(Write::Update(UpdateQuery {
                full_table_id: FullTableId::from((schema_id, table_id)),
    ...
    fn number_to_string_not_enough_length() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                vec![SCHEMA, TABLE],
                vec![("col", ast::Expr::Value(ast::Value::Number(BigDecimal::from(123))))]
            )),
            Ok(QueryAnalysis::Write(Write::Update(UpdateQuery {
                full_table_id: FullTableId::from((schema_id, table_id)),
                sql_types: vec![SqlType::Char(1)],
...
mod not_supported_values {
    ...
    fn national_strings() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                ...
                vec![(
                    "col",
                    ast::Expr::Value(ast::Value::NationalStringLiteral("str".to_owned()))
                )]
            )),
    ...
    fn hex_strings() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                vec![SCHEMA, TABLE],
                vec![("col", ast::Expr::Value(ast::Value::HexStringLiteral("str".to_owned())))]
            )),
    ...
    fn time_intervals() {
        ...
        assert_eq!(
            analyzer.analyze(update_statement(
                ...
                vec![(
                    "col",
                    ast::Expr::Value(ast::Value::Interval {
