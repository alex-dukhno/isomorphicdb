...
fn delete_statement(table_name: Vec<&'static str>) -> ast::Statement {
...
fn delete_all_from_table() {
    ...
    let analyzer = Analyzer::new(data_definition);
    assert_eq!(
        analyzer.analyze(ast::Statement::Delete {
            table_name: ast::ObjectName(vec![ident(SCHEMA), ident(TABLE)]),
            selection: None
        }),
