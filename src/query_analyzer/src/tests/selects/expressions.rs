...
fn select_specified_column_from_table() {
    ...
    assert_eq!(
        analyzer.analyze(select_with_columns(
            vec![SCHEMA, TABLE],
            vec![ast::SelectItem::UnnamedExpr(ast::Expr::Identifier(ident("col1")))]
        )),
        Ok(QueryAnalysis::Read(SelectQuery {
            full_table_id: FullTableId::from((schema_id, table_id)),
            projection_items: vec![ProjectionTreeNode::Item(Operator::Column {
...
fn select_column_that_is_not_in_table() {
    ...
    assert_eq!(
        analyzer.analyze(select_with_columns(
            vec![SCHEMA, TABLE],
            vec![ast::SelectItem::UnnamedExpr(ast::Expr::Identifier(ident("col2")))]
        )),
...
fn select_from_table_with_constant() {
    ...
    assert_eq!(
        analyzer.analyze(select_with_columns(
            vec![SCHEMA, TABLE],
            vec![ast::SelectItem::UnnamedExpr(ast::Expr::Value(number(1)))],
        )),
        Ok(QueryAnalysis::Read(SelectQuery {
            full_table_id: FullTableId::from((schema_id, table_id)),
...
fn select_parameters_from_a_table() {
    ...
    assert_eq!(
        analyzer.analyze(select_with_columns(
            vec![SCHEMA, TABLE],
            vec![ast::SelectItem::UnnamedExpr(ast::Expr::Identifier(ident("$1")))],
        )),
        Ok(QueryAnalysis::Read(SelectQuery {
            full_table_id: FullTableId::from((schema_id, table_id)),
...
mod multiple_values {
    ...
    fn select_value_as_expression_with_operation(
        ...
        select_with_columns(
            vec![SCHEMA, TABLE],
            vec![ast::SelectItem::UnnamedExpr(ast::Expr::BinaryOp {
                left: Box::new(left),
                op,
                right: Box::new(right),
            })],
