...
fn insert_with_parameters(full_name: Vec<&'static str>, parameters: Vec<&'static str>) -> ast::Statement {
    insert_with_values(
        full_name,
        vec![parameters.into_iter().map(ident).map(ast::Expr::Identifier).collect()],
    )
}
...
fn insert_into_table_with_parameters_and_values() {
    ...
    assert_eq!(
        analyzer.analyze(insert_with_values(
            vec![SCHEMA, TABLE],
            vec![vec![ast::Expr::Identifier(ident("$1")), ast::Expr::Value(number(1))]]
        )),
        Ok(QueryAnalysis::Write(Write::Insert(InsertQuery {
            full_table_id: FullTableId::from((schema_id, table_id)),
            column_types: vec![SqlType::SmallInt, SqlType::SmallInt],
