...
mod type_casting {
    ...
    fn to_boolean() {
        assert_eq!(
            ExecutableTree::UnOp {
                ...
                item: Box::new(ExecutableTree::Item(ExecutableItem::Const(ExecutableValue::StringLiteral(
                    "t".to_owned()
                ))))
            }
            .eval(&[], &[]),
    ...
    fn to_boolean_error() {
        assert_eq!(
            ...
            .eval(&[], &[]),
            Err(QueryExecutionError::invalid_text_representation(SqlType::Bool, &"abc"))
        )
    }
}
...
mod unary_minus {
    ...
    fn with_numbers() {
        assert_eq!(
            ExecutableTree::UnOp {
                ...
                item: Box::new(ExecutableTree::Item(ExecutableItem::Const(ExecutableValue::Int(32767)))),
            }
            .eval(&[], &[]),
            Ok(ScalarValue::Integer(-32767))
        );

        assert_eq!(
            ExecutableTree::UnOp {
                op: UnOperator::Arithmetic(UnArithmetic::Neg),
                item: Box::new(ExecutableTree::Item(ExecutableItem::Const(ExecutableValue::BigInt(32768)))),
            }
            .eval(&[], &[]),
            Ok(ScalarValue::BigInt(-32768))
        );
    }
    ...
    fn doubly_applied() {
        assert_eq!(
            ExecutableTree::UnOp {
                op: UnOperator::Arithmetic(UnArithmetic::Neg),
                item: Box::new(ExecutableTree::UnOp {
                    op: UnOperator::Arithmetic(UnArithmetic::Neg),
                    item: Box::new(ExecutableTree::Item(ExecutableItem::Const(ExecutableValue::Int(32767)))),
                })
            }
    ...
    fn with_string() {
        assert_eq!(
            ...
            .eval(&[], &[]),
            Err(QueryExecutionError::ambiguous_function(
