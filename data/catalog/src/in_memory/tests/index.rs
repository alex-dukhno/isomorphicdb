...
fn create_index_where_table_not_found() {
    let database = database();
    database.execute(SchemaChange::CreateSchema(CreateSchemaQuery { schema_name: SchemaName::from(&SCHEMA), if_not_exists: false })).unwrap();
...
fn create_index_for_table() {
    ...
    assert_eq!(database.index_exists(&FullTableName::from((&SCHEMA, &TABLE)), "index_name"), true);
}
...
fn index_data_is_populated_when_insert_data_into_table_with_single_column() {
    ...
    database.work_with(&FullTableName::from((&SCHEMA, &TABLE)), |table| table.insert(&[vec![Some(StaticTypedTree::Item(StaticTypedItem::Const(TypedValue::SmallInt(1))))]]));

    assert_eq!(database.work_with_index((SCHEMA.to_owned(), TABLE.to_owned(), "index_name".to_owned()), |index| index.select().map(|(value, _record_id)| value).collect::<Vec<Binary>>()), vec![Binary::pack(&[Datum::from_i16(1)])])
}
...
fn index_of_single_column_data_is_populated_when_insert_data_into_table_with_many_columns() {
    ...
    database.execute(SchemaChange::CreateTable(CreateTableQuery {
        full_table_name: FullTableName::from((&SCHEMA, &TABLE)),
        column_defs: vec![ColumnInfo { name: "col_1".to_owned(), sql_type: SqlType::small_int() }, ColumnInfo { name: "col_2".to_owned(), sql_type: SqlType::small_int() }, ColumnInfo { name: "col_3".to_owned(), sql_type: SqlType::small_int() }],
        if_not_exists: false
    })).unwrap();
    database.execute(SchemaChange::CreateIndex(CreateIndexQuery {
        name: "index_name".to_owned(),
    ...
    database.work_with(&FullTableName::from((&SCHEMA, &TABLE)), |table| table.insert(&[vec![
    ...
    assert_eq!(database.work_with_index((SCHEMA.to_owned(), TABLE.to_owned(), "index_name".to_owned()), |index| index.select().map(|(value, _record_id)| value).collect::<Vec<Binary>>()), vec![Binary::pack(&[Datum::from_i16(2)]), Binary::pack(&[Datum::from_i16(5)]), Binary::pack(&[Datum::from_i16(8)])]);
}
...
fn all_indexes_are_populated_when_insert_data_into_table_with_many_columns() {
    ...
    assert_eq!(database.work_with_index((SCHEMA.to_owned(), TABLE.to_owned(), "index_name_1".to_owned()), |index| index.select().map(|(value, _record_id)| value).collect::<Vec<Binary>>()), vec![Binary::pack(&[Datum::from_i16(2)]), Binary::pack(&[Datum::from_i16(5)]), Binary::pack(&[Datum::from_i16(8)])]);
    assert_eq!(database.work_with_index((SCHEMA.to_owned(), TABLE.to_owned(), "index_name_2".to_owned()), |index| index.select().map(|(value, _record_id)| value).collect::<Vec<Binary>>()), vec![Binary::pack(&[Datum::from_i16(1)]), Binary::pack(&[Datum::from_i16(4)]), Binary::pack(&[Datum::from_i16(7)])]);
}
