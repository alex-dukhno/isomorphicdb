...
pub struct CatalogHandlerOld<'c> {
    database: TransactionalDatabase<'c>,
}

impl<'c> From<TransactionalDatabase<'c>> for CatalogHandlerOld<'c> {
    fn from(database: TransactionalDatabase<'c>) -> CatalogHandlerOld {
        CatalogHandlerOld { database }
    }
}

impl<'c> CatalogHandlerOld<'c> {
    pub fn schema_exists(&self, schema_name: &SchemaName) -> bool {
        self.database
            .lookup_table_ref(format!("{}.{}", DEFINITION_SCHEMA, SCHEMATA_TABLE))
            .scan()
