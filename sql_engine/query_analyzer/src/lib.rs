        ...
        match &statement {
                ...
                Ok(full_table_name) => match self.database.table_definition(full_table_name.clone()) {
                    ...
                    Some(Some(table_info)) => {
                        ...
                        let values = match body {
                            ...
                            sql_ast::SetExpr::Insert(_) => unimplemented!(),
            ...
            sql_ast::Statement::Delete { table_name, .. } => match FullTableName::try_from(table_name) {
                ...
                Ok(full_table_name) => match self.database.table_definition(full_table_name.clone()) {
                    None => Err(AnalysisError::schema_does_not_exist(full_table_name.schema())),
                    Some(None) => Err(AnalysisError::table_does_not_exist(full_table_name)),
                    Some(Some(_table_info)) => Ok(QueryAnalysis::DML(UntypedQuery::Delete(UntypedDeleteQuery {
                        full_table_name,
                    }))),
                },
            },
            ...
            sql_ast::Statement::Truncate { .. } => unimplemented!(),
            sql_ast::Statement::Msck { .. } => unimplemented!(),
            sql_ast::Statement::Directory { .. } => unimplemented!(),
            sql_ast::Statement::CreateDatabase { .. } => unimplemented!(),
        }
    }
}
